...
| 场景 | 用什么 | 原因 |
|---|---|---|
| **§1 知识地图** | **Mermaid** | `flowchart TD` 自动生成布局 |
| **§3.4 概念定位图** | **Mermaid** | 层级关系图的强项 |
| **§3.3 推导链** | **SVG** | 需要精确控制每步位置和高亮区域 |
| **§4 例题过程图** | **SVG** | 每步布局需要精确控制 |
| 其他场景 | agent 自判 | 能用 Mermaid 就用 Mermaid |

**一句话规则：拓扑类图（谁包含谁、谁依赖谁）用 Mermaid，精确布局图（推导步骤、过程流程）用 SVG。**
...
**Agent 判断覆盖**：如果 agent 认为手写 SVG 比 Mermaid 更精确（比如节点 <15 但布局需要特殊排列），可以直接跳过 Mermaid 管线，直接写 SVG。上面的数字是参考，不是硬约束。
